...
func Unpack(strInput string) (string, error) {
	...
	for i, r := range runeInput {
		...
		if i == 0 && unicode.IsDigit(r) && IsArabicDigit(r) {
		...
		// Error! число
		if unicode.IsDigit(r) && IsArabicDigit(r) && unicode.IsDigit(runeInput[i-1]) && IsArabicDigit(runeInput[i-1]) {
			return "", ErrInvalidString
		}

		// Success! Repeat! текущая руна цифра, а предыдущая руна символ или экран
		if unicode.IsDigit(r) && IsArabicDigit(r) {
			...
			} else if unicode.IsDigit(r) && (!unicode.IsDigit(runeInput[i-1]) || !IsArabicDigit(runeInput[i-1])) {
		...
		if (!unicode.IsDigit(r) || !IsArabicDigit(r)) &&
			(!unicode.IsDigit(runeInput[i-1]) || !IsArabicDigit(runeInput[i-1])) {
			runeOutput = append(runeOutput, runeInput[i-1])
		}

		// Success! последняя руна символ в слайсе
		if (!unicode.IsDigit(r) || !IsArabicDigit(r)) && (len(runeInput)-1) == i {
